...
import { KPICardsProps, DashboardMetrics } from '@/lib/dashboard-types';
...
const KPICard: React.FC<KPICardProps> = ({ 
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-lg hover:border-gray-300 transition-all duration-300 ease-in-out transform hover:-translate-y-1 p-4 sm:p-6 card-hover card-polish">
      <div className="flex items-center">
        <div className={`p-2 sm:p-3 rounded-lg ${iconBgColor} transition-all duration-200 ease-in-out`}>
          <div className={`${iconTextColor} transition-colors duration-200`}>
            {icon}
          </div>
        </div>
        <div className="ml-3 sm:ml-4 flex-1 min-w-0">
          <p className="text-xs sm:text-sm font-medium text-gray-600 mb-1 transition-colors duration-200 truncate">{title}</p>
          <p className="text-xl sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2 transition-all duration-300 ease-in-out">
            {formatValue(value)}
          </p>
          {trend && (
            <div className="flex items-center text-xs sm:text-sm">
              <div className={`flex items-center ${getTrendColor()} transition-colors duration-200`}>
...
const KPICards: React.FC<KPICardsProps> = ({ metrics, isLoading = false }) => {
  ...
  const getTrendData = (metricType: string) => {
    const trends = {
      activeProjects: { direction: 'up' as const, percentage: 12, description: 'vs mes anterior' },
      activePersonnel: { direction: 'stable' as const, percentage: 0, description: 'sin cambios' },
      pendingTasks: { direction: 'down' as const, percentage: 8, description: 'vs semana anterior' },
      remainingBudget: { direction: 'up' as const, percentage: 5, description: 'vs proyectado' },
    };
